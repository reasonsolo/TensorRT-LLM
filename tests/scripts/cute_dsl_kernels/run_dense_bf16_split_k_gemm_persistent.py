...
custom op for split-K tactics:
  ...
  1. ``RubinBf16PersistentDenseGemmKernel(split_k_slices=S)`` writes FP32 partials
     into a workspace shaped ``[M, N, S]`` (the static persistent scheduler
     enumerates ``S`` L-tiles, decoding batch/split from the L coordinate).
  2. ``gemm.split_k_reduction`` sums the ``S`` partials into the output.
...
def run(
    ...
    if split_k_slices > 1:
        workspace = torch.empty(split_k_slices, m, n, dtype=torch.float32, device=device)
        ws_tmp = workspace.permute(1, 2, 0)  # [M, N, splits]
        c_tmp = c.unsqueeze(-1)  # [M, N, 1]
        # GEMM workspace store needs the dynamic-leading-dim layout.
        ws_cute = from_dlpack(ws_tmp, assumed_align=16).mark_layout_dynamic(leading_dim=1)
        # Reduction reads contiguous, freshly-allocated buffers: keep their
        # layout fully static so the universal copy widens to 128-bit LDG/STG.
        ws_cute_static = from_dlpack(ws_tmp, assumed_align=16)
        c_cute_static = from_dlpack(c_tmp, assumed_align=16)
        ...
        def run_kernel():
            compiled_gemm(m, n, k, batch, a_ptr, b_ptr, ws_cute, stream)
            compiled_reduction(ws_cute_static, c_cute_static, stream)
    else:
        c_tmp = c.unsqueeze(-1)  # [M, N, 1]
        c_cute = from_dlpack(c_tmp).mark_layout_dynamic(leading_dim=1)
        compiled_gemm = cute.compile(
            gemm.wrapper,
            m,
            n,
        ...
        def run_kernel():
            compiled_gemm(m, n, k, batch, a_ptr, b_ptr, c_cute, stream)
