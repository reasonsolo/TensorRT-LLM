...
class _FakeEvictablePage:
    def __init__(self, life_cycle: LifeCycleId, ugpu_id: int | None) -> None:
        self.life_cycle = life_cycle
        self.ugpu_id = ugpu_id
...
class TestStorageManagerInit(unittest.TestCase):
    ...
    def test_ugpu_disabled_by_default_when_localization_supported(self) -> None:
        """Default GPU tier config stays non-localized even on uGPU-capable hardware."""
        ...
        self.assertEqual(gpu_storage.num_ugpus, 1)
...
class TestSlotAllocatorOffset(unittest.TestCase):
    ...
    # ------------------------------------------------------------------
    # Test 2: ugpu1 offset — slot_ids count up from LOCALIZATION_OFFSET
    # ------------------------------------------------------------------
    def test_ugpu1_slot_ids_start_at_localization_offset(self) -> None:
    ...
    def test_consecutive_ugpu1_slot_ids_increment_from_offset(self) -> None:
        """Consecutive ugpu1 slot_ids are LOCALIZATION_OFFSET, +1, +2, …."""
        alloc = SlotAllocator(capacity=4, slot_id_offset=LOCALIZATION_OFFSET)
    ...
    def test_local_idx_strips_offset(self) -> None:
        """_local_idx(slot_id) returns the 0-based per-pool index for both ugpu0 and ugpu1."""
...
class TestStorageManagerLocalized(unittest.TestCase):
    """Tests for StorageManager when GpuCacheLevelStorage has num_ugpus > 1.

    We enable uGPU in the GPU tier config and set
    ...
    CacheLevelManager._create_cache_level_storage
      → GpuCacheLevelStorage(localized=True)
        → GpuPoolGroup (one SlotAllocator per uGPU, ugpu1 offset by a canonical slot stride)

    Tests cover:
    - Correct storage type instantiation (num_ugpus > 1) when uGPU is enabled and supported
    - Assertion guards: ugpu_id must not be None for localized allocation operations
      (new_slots, new_slots_for_pool_group)
    - Slot id encoding: ugpu0 slots below the localized slot-id offset, ugpu1 slots at or above it
    - Slot.ugpu_id is stamped correctly by the allocator
    - num_slots returns the per-uGPU count and both uGPUs receive equal quota
    - release_slot uses slot.ugpu_id to dispatch to the correct uGPU allocator
    """
    ...
    def test_localized_storage_type_created_when_supported(self) -> None:
        """CacheLevelManager must create num_ugpus > 1 when uGPU is enabled and supported."""
        self.storage_manager = self._make_localized_sm(num_layers=2)
        gpu_storage = self.storage_manager._levels[GPU_LEVEL].storage
        self.assertIsInstance(gpu_storage, GpuCacheLevelStorage)
        self.assertGreater(gpu_storage.num_ugpus, 1)

    # ------------------------------------------------------------------
    # Test 2: assertion guards — ugpu_id required at GPU level
    # ------------------------------------------------------------------
    def test_new_slots_asserts_without_ugpu_id(self) -> None:
        """new_slots at GPU level must raise AssertionError when ugpu_id is omitted."""
        self.storage_manager = self._make_localized_sm(num_layers=2)
        sm = self.storage_manager
    ...
    def test_new_slots_for_pool_group_asserts_without_ugpu_id(self) -> None:
        """new_slots_for_pool_group at GPU level must raise AssertionError when ugpu_id is omitted."""
        self.storage_manager = self._make_localized_sm(num_layers=2)
        sm = self.storage_manager
        with self.assertRaises(AssertionError):
            sm.new_slots_for_pool_group(GPU_LEVEL, PoolGroupIndex(0), 1)

    def test_num_slots_asserts_without_ugpu_id(self) -> None:
        """num_slots at GPU level must raise AssertionError when ugpu_id is omitted."""
        self.storage_manager = self._make_localized_sm(num_layers=2)
        with self.assertRaises(AssertionError):
            self.storage_manager.num_slots(PoolGroupIndex(0))

    # ------------------------------------------------------------------
    # Test 3: both uGPUs receive equal slot counts (symmetric quota split)
    # ------------------------------------------------------------------
    def test_num_slots_per_ugpu_are_equal(self) -> None:
        """GpuCacheLevelStorage splits quota evenly; both uGPUs get the same slot count."""
        self.storage_manager = self._make_localized_sm(num_layers=2)
        sm = self.storage_manager
        pg = PoolGroupIndex(0)
        n0 = sm.num_slots(pg, GPU_LEVEL, ugpu_id=0)
        n1 = sm.num_slots(pg, GPU_LEVEL, ugpu_id=1)
        self.assertGreater(n0, 0)
        self.assertEqual(n0, n1, "Both uGPUs must receive the same quota")

    def test_localized_total_quota_respects_global_budget(self) -> None:
        """Localized storage must split the configured GPU quota across uGPUs."""
    ...
    # ------------------------------------------------------------------
    # Test 4: slot_id encoding — ugpu0 below localized slot offset, ugpu1 at or above it
    # ------------------------------------------------------------------
    def test_ugpu0_slot_ids_below_localization_offset(self) -> None:
        """Slots allocated from ugpu0 must have slot_id below the localized slot offset."""
        self.storage_manager = self._make_localized_sm(num_layers=2)
        sm = self.storage_manager
        pg = PoolGroupIndex(0)
        storage = sm._levels[GPU_LEVEL].storage
        localized_offset = _get_localized_slot_id_offset(sm, pg)
        slots = sm.new_slots_for_pool_group(GPU_LEVEL, pg, 2, ugpu_id=0)
        try:
            for slot in slots:
                self.assertLess(int(slot.slot_id), localized_offset)
                self.assertEqual(slot.ugpu_id, 0)
        finally:
            for slot in slots:
                storage.release(pg, slot, slot.ugpu_id)

    def test_ugpu1_slot_ids_at_or_above_localization_offset(self) -> None:
        """Slots allocated from ugpu1 must have slot_id at or above the localized slot offset."""
        self.storage_manager = self._make_localized_sm(num_layers=2)
        sm = self.storage_manager
        pg = PoolGroupIndex(0)
        storage = sm._levels[GPU_LEVEL].storage
        localized_offset = _get_localized_slot_id_offset(sm, pg)
        slots = sm.new_slots_for_pool_group(GPU_LEVEL, pg, 2, ugpu_id=1)
        try:
            for slot in slots:
                self.assertGreaterEqual(int(slot.slot_id), localized_offset)
                self.assertEqual(slot.ugpu_id, 1)
        finally:
            for slot in slots:
                storage.release(pg, slot, slot.ugpu_id)
    ...
    def test_localized_page_index_matches_slot_address(self) -> None:
        """Shared-pool-pointer execution must encode the full uGPU displacement in page indices."""
        self.storage_manager = self._make_localized_sm(num_layers=2)
        sm = self.storage_manager
        pg = PoolGroupIndex(0)
        gpu_storage = sm._levels[GPU_LEVEL].storage
        slot = sm.new_slots_for_pool_group(GPU_LEVEL, pg, 1, ugpu_id=1)[0]
        try:
            layer_id = LayerId(0)
            role = DataRole("key")
            ...
            gpu_storage.release(pg, slot, slot.ugpu_id)

    # ------------------------------------------------------------------
    # Test 5: release_slot uses slot.ugpu_id (dispatches to the correct allocator)
    # ------------------------------------------------------------------
    def test_release_slot_restores_free_count(self) -> None:
        """Allocating a slot on ugpu1 then releasing it must restore free count.

        This verifies that release_slot forwards slot.ugpu_id to the localized
        ...
        slots = sm.new_slots_for_pool_group(GPU_LEVEL, pg, 1, ugpu_id=1)
        self.assertEqual(
    ...
    def test_eviction_filter_selects_target_ugpu(self) -> None:
        """Localized eviction must only free pages from the uGPU being allocated."""
        ctrl = PerLevelEvictionController([PoolGroupIndex(0)], GPU_LEVEL)
        page0 = _FakeEvictablePage(LifeCycleId(0), ugpu_id=0)
        page1 = _FakeEvictablePage(LifeCycleId(0), ugpu_id=1)
        ctrl.schedule_for_eviction(page0)
        ctrl.schedule_for_eviction(page1)

        evicted = ctrl.evict([1], lambda page: page.ugpu_id == 1)

        self.assertEqual(evicted[PoolGroupIndex(0)], [page1])
        self.assertIsNone(page1.node_ref)
        self.assertIsNotNone(page0.node_ref)
        self.assertEqual(
            ctrl.num_evictable_pages(PoolGroupIndex(0), lambda page: page.ugpu_id == 0),
            1,
        )
