...
using namespace tensorrt_llm::ugpu;

// Memory allocation type for dual-stream tests
enum class MemoryAllocationType
{
    SAME,      // Allocate on the same UGPU as the stream
    DIFFERENT, // Allocate on a different UGPU from the stream
    NORMAL     // Use regular cudaMalloc (no UGPU localization)
};
...
class UgpuLocalizationTest : public ::testing::Test
{
protected:
    ...
    {
        UgpuLocalizationHandle* handle = nullptr;
        ...
        void allocate(UgpuLocalizationHandle* newHandle, size_t size, MemoryAllocationType newType, int streamUgpuId)
        {
            reset();
            handle = newHandle;
            type = newType;
            switch (type)
            {
            case MemoryAllocationType::SAME: handle->ugpuMalloc(&ptr, size, streamUgpuId); break;
            case MemoryAllocationType::DIFFERENT:
                handle->ugpuMalloc(&ptr, size, 1 - streamUgpuId); // 0->1, 1->0
                break;
        ...
        void resetNoThrow() noexcept
        {
            ...
            else
            {
                ...
                {
                    ADD_FAILURE() << "ugpuFree failed during cleanup";
                }
            }
            ptr = nullptr;
        }
    ...
    struct StreamHolder
    {
        ...
        ~StreamHolder() noexcept
        {
            if (stream != nullptr)
            {
                auto result = cudaStreamDestroy(reinterpret_cast<cudaStream_t>(stream));
                if (result != cudaSuccess)
                {
    ...
    std::unique_ptr<UgpuLocalizationHandle> mHandle;
    bool mSupportsUgpu = false;

    void SetUp() override
    {
        // Create handle
        mHandle = std::make_unique<UgpuLocalizationHandle>();

        // Check if UGPU is supported
        mSupportsUgpu = mHandle->supportsUgpuLocalization();
        if (!mSupportsUgpu)
        {
            TLLM_LOG_WARNING("UGPU localization is not supported on this device, skipping UGPU-specific tests.");
        }
    }
    ...
    void runDualStreamPerformanceTest(MemoryAllocationType srcType, MemoryAllocationType dstType, char const* testName)
    {
        ...
        // Allocate memory for stream 0 (UGPU 0)
        ...
        // Allocate memory for stream 1 (UGPU 1)
        ...
        // Create two localized streams on different UGPUs
        StreamHolder stream0{mHandle->createUgpuLocalizedStream(0)};
        StreamHolder stream1{mHandle->createUgpuLocalizedStream(1)};
...
TEST_F(UgpuLocalizationTest, CreateAndDestroyHandle)
{
...
// Test 3: Allocate and free memory on UGPU 0
TEST_F(UgpuLocalizationTest, MallocFreeOnUgpu0)
{
    if (!mSupportsUgpu)
    {
        GTEST_SKIP() << "UGPU not supported, skipping test";
    }

    void* devPtr = nullptr;
    size_t size = 1024 * 1024; // 1 MB

    // Allocate memory on UGPU 0
    ASSERT_NO_THROW(mHandle->ugpuMalloc(&devPtr, size, 0));
    ASSERT_NE(devPtr, nullptr);

    // Free memory
    ASSERT_NO_THROW(mHandle->ugpuFree(devPtr));
}

// Test 4: Allocate and free memory on UGPU 1
TEST_F(UgpuLocalizationTest, MallocFreeOnUgpu1)
{
    if (!mSupportsUgpu)
    {
        GTEST_SKIP() << "UGPU not supported, skipping test";
    ...
    // Allocate memory on UGPU 1
    ASSERT_NO_THROW(mHandle->ugpuMalloc(&devPtr, size, 1));
    ASSERT_NE(devPtr, nullptr);
...
// Test 5: Create localized stream on UGPU 0
TEST_F(UgpuLocalizationTest, CreateLocalizedStreamUgpu0)
{
    if (!mSupportsUgpu)
    {
        GTEST_SKIP() << "UGPU not supported, skipping test";
    ...
    // Create stream localized to UGPU 0
    CUstream stream = mHandle->createUgpuLocalizedStream(0);
    ASSERT_NE(stream, nullptr);

    TLLM_CUDA_CHECK(cudaStreamDestroy(reinterpret_cast<cudaStream_t>(stream)));
}

// Test 6: Create localized stream on UGPU 1
TEST_F(UgpuLocalizationTest, CreateLocalizedStreamUgpu1)
{
    if (!mSupportsUgpu)
    {
        GTEST_SKIP() << "UGPU not supported, skipping test";
    ...
    // Create stream localized to UGPU 1
    CUstream stream = mHandle->createUgpuLocalizedStream(1);
    ASSERT_NE(stream, nullptr);
...
// Run them explicitly with --gtest_also_run_disabled_tests when benchmarking uGPU locality.

// Performance Test 1: Single stream, copy 20GB memory
TEST_F(UgpuLocalizationTest, DISABLED_PerformanceTestSingleStream20GB)
{
...
// Performance Test 2: Dual streams, source and destination on the same UGPU as stream
TEST_F(UgpuLocalizationTest, DISABLED_PerformanceTestDualStreamSameUgpu)
{
    if (!mSupportsUgpu)
    {
        GTEST_SKIP() << "UGPU not supported, skipping test";
    ...
    runDualStreamPerformanceTest(
        MemoryAllocationType::SAME, MemoryAllocationType::SAME, "Performance Test 2 - Dual Stream Same UGPU (2x10GB)");
}

// Performance Test 3: Dual streams, source on same UGPUs as stream and destination on different UGPUs
TEST_F(UgpuLocalizationTest, DISABLED_PerformanceTestDualStreamSrcSameDstDifferentUgpu)
{
    if (!mSupportsUgpu)
    {
        GTEST_SKIP() << "UGPU not supported, skipping test";
    ...
    runDualStreamPerformanceTest(MemoryAllocationType::SAME, MemoryAllocationType::DIFFERENT,
        "Performance Test 3 - Dual Stream Src Same Dst Different UGPU (2x10GB)");
}

// Performance Test 3: Dual streams, source on different UGPUs as stream and destination on same UGPUs
TEST_F(UgpuLocalizationTest, DISABLED_PerformanceTestDualStreamSrcDifferentDstSameUgpu)
{
    if (!mSupportsUgpu)
    {
        GTEST_SKIP() << "UGPU not supported, skipping test";
    ...
    runDualStreamPerformanceTest(MemoryAllocationType::DIFFERENT, MemoryAllocationType::SAME,
        "Performance Test 3 - Dual Stream Src Different Dst Same UGPU (2x10GB)");
}

// Performance Test 3: Dual streams, source and destination on different UGPUs as stream
TEST_F(UgpuLocalizationTest, DISABLED_PerformanceTestDualStreamDifferentUgpu)
{
    if (!mSupportsUgpu)
    {
        GTEST_SKIP() << "UGPU not supported, skipping test";
    ...
    runDualStreamPerformanceTest(MemoryAllocationType::DIFFERENT, MemoryAllocationType::DIFFERENT,
        "Performance Test 3 - Dual Stream Src and Dst Different UGPU (2x10GB)");
}

// Performance Test 4: Dual localized streams with regular cudaMalloc memory
TEST_F(UgpuLocalizationTest, DISABLED_PerformanceTestDualLocalizedStreamRegularMemory)
{
    if (!mSupportsUgpu)
    {
        GTEST_SKIP() << "UGPU not supported, skipping test";
