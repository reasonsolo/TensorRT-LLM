...
#define INST_BIGFUSE(NS, BS)                                                                                           \
    template __global__ void mhcBigFuseKernel<NS, BS, /*kFuseNorm=*/false, /*kUseTma=*/false>(float const*,            \
        float const*, __nv_bfloat16 const*, float const*, float const*, float*, float*, __nv_bfloat16*, int, int, int, \
        float, float, float, float, int, __nv_bfloat16 const*, float);                                                 \
    template __global__ void mhcBigFuseKernel<NS, BS, /*kFuseNorm=*/true, /*kUseTma=*/false>(float const*,             \
        float const*, __nv_bfloat16 const*, float const*, float const*, float*, float*, __nv_bfloat16*, int, int, int, \
        float, float, float, float, int, __nv_bfloat16 const*, float);

#define INST_BIGFUSE_TMA(FN)                                                                                           \
    template __global__ void mhcBigFuseKernel<1, 512, /*kFuseNorm=*/FN, /*kUseTma=*/true>(float const*, float const*,  \
