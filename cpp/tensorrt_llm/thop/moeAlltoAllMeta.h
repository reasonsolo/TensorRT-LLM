...
enum MoeA2AMetaInfoIndex : int64_t
{
    ...
    // Double-buffered Lamport sync slots for CFT dispatch (2 * ep_size int32s per rank).
    LAMPORT_BUF_OFFSET_INDEX = 11,
    PAYLOAD_DATA_OFFSET_INDEX = 12,
    // Static max tokens/rank (a count, not a byte offset).
    MAX_NUM_TOKENS_INDEX = 13,
    NUM_METAINFO_FIELDS = 14
};
...
inline std::vector<std::pair<char const*, int64_t>> getMoeA2AMetaInfoIndexPairs()
{
    return {
        ...
        {"MOE_A2A_LAMPORT_BUF_OFFSET_INDEX", LAMPORT_BUF_OFFSET_INDEX},
