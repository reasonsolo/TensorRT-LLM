    ...
    def _create_cache_level_storage(
        ...
        match config.tier:
            case CacheTier.GPU_MEM:
                ...
                localized = config.enable_ugpu and is_device_localization_supported(
                    get_current_device_id()
                )
...
class StorageManager:
    ...
    def new_slots(
        ...
        localized_gpu_storage = isinstance(storage, GpuCacheLevelStorage) and storage.num_ugpus > 1
        assert level != GPU_LEVEL or not localized_gpu_storage or ugpu_id is not None, (
            "ugpu_id must be provided when gpu storage supports localization"
        )

        def free_slots(pg: PoolGroupIndex) -> int:
            if ugpu_id is not None:
                return storage.get_num_free_slots(pg, ugpu_id)
        ...
        try:
            for life_cycle in typed_range(self.num_life_cycles):
                ...
                if ugpu_id is not None:
                    ret[life_cycle] = storage.allocate_multiple(
                        pg_idx, num_slots[life_cycle], ugpu_id
                    )
                else:
            ...
            for lc, slots in typed_enumerate(ret):
                ...
                for s in slots:
                    if ugpu_id is not None:
                        storage.release(pg_idx, s, ugpu_id)
                    else:
    ...
    def new_slots_for_pool_group(
        ...
        def free_slots() -> int:
            if ugpu_id is not None:
                return storage.get_num_free_slots(pg_idx, ugpu_id)
        ...
        try:
            if ugpu_id is not None:
                return storage.allocate_multiple(pg_idx, num_slots, ugpu_id)
    ...
    def _prepare_free_slots(
        ...
        def page_matches_target_ugpu(page: EvictablePage) -> bool:
            return not localized_gpu_storage or page.ugpu_id == ugpu_id

        page_filter = page_matches_target_ugpu if localized_gpu_storage else None

        def get_free_slots(pg: PoolGroupIndex, ugpu_id: int | None = None) -> int:
            if localized_gpu_storage:
                return storage.get_num_free_slots(pg, ugpu_id)
            return storage.get_num_free_slots(pg)
        ...
        for pg_idx in typed_range(self.num_pool_groups):
            ...
            old_free_cnt = get_free_slots(pg_idx, ugpu_id)
        ...
        if is_last_level:
            for pg_idx in typed_range(self.num_pool_groups):
                ...
                new_free_cnt = get_free_slots(pg_idx, ugpu_id)
    ...
    def _batched_migrate(
        ...
        - Cross-tier migration (e.g. GPU → Host): the destination is always a
          ...
          default ugpu_id=0 — correct because Host/Disk pools are not
          uGPU-aware.
        - Same-level defragmentation: moves slots within the same tier and
          same uGPU to eliminate fragmentation after a shrink.

        Not supported:

        - Same-level cross-uGPU migration (moving data between uGPUs within
          the GPU tier).  If added, dst_pool_group operations
          (.num_free_slots, .allocate_multiple, .slot_address) would need an
          explicit target ugpu_id.
        """
        ...
        try:
            ...
            for src, dst in zip(src_pages, dst_slots):
                ...
                if isinstance(src_pool_group, GpuPoolGroup):
                    src_addresses = src_pool_group.slot_address(src.slot_id, src.ugpu_id)
                else:
            ...
            for src, dst in zip(src_pages, dst_slots):
                ...
                if update_src:
                    ...
                    if isinstance(src_pool_group, GpuPoolGroup):
                        src_pool_group.release(src, src.ugpu_id)
                    else:
    ...
    def num_slots(
        ...
        assert (
            ...
            or not (isinstance(storage, GpuCacheLevelStorage) and storage.num_ugpus > 1)
            or ugpu_id is not None
        ), "ugpu_id must be provided when gpu storage supports localization"
        if isinstance(storage, GpuCacheLevelStorage) and storage.num_ugpus > 1:
            return storage.num_slots(pool_group_index, ugpu_id)
    ...
    def release_slot(self, life_cycle: LifeCycleId, cache_level: CacheLevel, slot: Slot) -> None:
        ...
        storage = self._levels[cache_level].storage
        if slot.ugpu_id is not None:
            storage.release(pg_idx, slot, slot.ugpu_id)
        else:
    ...
    def slot_address(
        ...
        storage = self._levels[level].storage
        if isinstance(storage, GpuCacheLevelStorage) and storage.num_ugpus > 1:
            ugpu_id = storage._pool_groups[pg_idx].get_ugpu_id(slot_id)
            return storage.slot_address(pg_idx, pool_idx, slot_id, ugpu_id)
    ...
    def _base_page_index_for_slot(self, life_cycle: LifeCycleId, slot_id: SlotId) -> PageIndex:
        storage = self._levels[GPU_LEVEL].storage
        if not (isinstance(storage, GpuCacheLevelStorage) and storage.num_ugpus > 1):
        ...
        # Shared-pool-pointer execution keeps the canonical uGPU0 pool base and
        # encodes the uGPU displacement in the same canonical slot-id space
        # used by the localized allocators.
        return PageIndex(local_idx + ugpu_id * pool_group._slot_id_offset)
    ...
    def get_statistics(
        ...
        for pg_idx in typed_range(self.num_pool_groups):
            ...
            if isinstance(pg, GpuPoolGroup):
                total = sum(pg.num_slots(uid) for uid in range(pg.num_ugpus))
                free = sum(pg.num_free_slots(uid) for uid in range(pg.num_ugpus))
            else:
                total = pg.num_slots
