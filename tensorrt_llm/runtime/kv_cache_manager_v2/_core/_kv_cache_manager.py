...
class KVCacheManager:
    ...
    def get_page_index_upper_bound(self, layer_id: LayerId, data_role: DataRole) -> int:
        ...
        # method(ugpu_id) on localized GpuPoolGroup.  Handle both.
        num_slots = (
        ...
        if (
            ...
            and pool_group.num_ugpus > 1
        ):
            # Shared-pool-pointer localized execution keeps uGPU0 as the
            # canonical base address and encodes the uGPU VA displacement in
            ...
            upper_bound += (pool_group.num_ugpus - 1) * scale * pool_group._slot_id_offset
        return upper_bound
    ...
    def create_kv_cache(
        ...
        ugpu_id: the uGPU to pin this KV cache to. None means non-localized.
        """
        if reuse_scope is None:
            reuse_scope = ReuseScope()
        assert type(reuse_scope) is ReuseScope
        if ugpu_id is not None:
            reuse_scope = reuse_scope._replace(ugpu_id=ugpu_id)
        reuse_match = (
    ...
    def num_ugpus(self) -> int:
        """Number of uGPUs. Returns 1 for non-localized (standard) configurations."""
        gpu_storage = self._storage._levels[GPU_LEVEL].storage
        if isinstance(gpu_storage, GpuCacheLevelStorage):
            return gpu_storage.num_ugpus
        return 1

    def get_per_ugpu_free_slots(self) -> list[int]:
        """Free slot count per uGPU, summed across all pool groups.

        Returns a list of length num_ugpus. For non-localized
        configurations (num_ugpus == 1), returns total free slots
    ...
    def clamp_max_seq_len_for_mem(self, batch_size: int, token_num_upper_bound: int) -> int:
        ...
        gpu_storage = storage._levels[GPU_LEVEL].storage
        use_primary_ugpu = (
            isinstance(gpu_storage, GpuCacheLevelStorage) and gpu_storage.num_ugpus > 1
        )
        remaining_slots = cast(
            TypedIndexList[PoolGroupIndex, int],
            [
                storage.num_slots(pg, ugpu_id=0) if use_primary_ugpu else storage.num_slots(pg)
