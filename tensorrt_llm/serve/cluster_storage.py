...
class ClusterStorage(abc.ABC):
    ...
    # return all keys that start with the key_prefix
...
class HttpClusterStorageServer(ClusterStorage):
    ...
    def add_routes(self, server: FastAPI):
        ...
        server.add_api_route("/get_keys",
                             jsonify(self.get_keys),
                             methods=["GET"])

    async def start(self):
        if self._check_expired_task:
            raise ValueError("Check expired task already started")
...
class HttpClusterStorageClient(ClusterStorage):
    ...
    async def get_keys(self, key_prefix: str) -> List[str]:
        keys = await self._get("get_keys", key_prefix=key_prefix)
