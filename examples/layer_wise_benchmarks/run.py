...
def main():
    ...
    parser.add_argument(
        "--moe-backend-for-prefill",
        type=str,
        choices=["CUTLASS", "DEEPGEMM", "WIDEEP", "CUTEDSL"],
    )
    parser.add_argument("--moe-max-num-tokens", type=int)
    parser.add_argument("--num-mtp-layers", type=int)
    group = parser.add_mutually_exclusive_group()
    group.add_argument(
    ...
    group = parser.add_mutually_exclusive_group()
    group.add_argument(
        "--use-cute-dsl-bf16-gemm", action="store_true", dest="use_cute_dsl_bf16_gemm"
    )
    group.add_argument(
        "--no-use-cute-dsl-bf16-gemm", action="store_false", dest="use_cute_dsl_bf16_gemm"
    )
    parser.set_defaults(use_cute_dsl_bf16_gemm=None)
    group = parser.add_mutually_exclusive_group()
    group.add_argument("--use-cute-dsl-bf16-bmm", action="store_true", dest="use_cute_dsl_bf16_bmm")
    group.add_argument(
        "--no-use-cute-dsl-bf16-bmm", action="store_false", dest="use_cute_dsl_bf16_bmm"
    )
    parser.set_defaults(use_cute_dsl_bf16_bmm=None)
    group = parser.add_mutually_exclusive_group()
    ...
    parser.add_argument(
        "--seq-len-kv-cache", type=comma_separated_ints, dest="seq_len_kv_cache_list"
    )
    ...
    group = parser.add_mutually_exclusive_group()
    group.add_argument(
        "--replay-verify-metadata", action="store_true", dest="replay_verify_metadata"
    )
    group.add_argument(
    ...
    if args.use_cute_dsl_bf16_gemm is None:
        args.use_cute_dsl_bf16_gemm = True
    if args.use_cute_dsl_bf16_bmm is None:
        args.use_cute_dsl_bf16_bmm = True
